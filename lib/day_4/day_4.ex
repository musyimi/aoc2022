defmodule Aoc2022.Day4 do
  ...
  defp dangerous_points(lines) do
    ...
    |> Enum.map(fn x_coordinate ->
      Enum.count(0..max_y, fn y_coordinate ->
       Enum.count_until(lines, &intersects?(&1, x_coordinate, y_coordinate), 2) == 2
  ...
  defp intersects?({{x1, y1}, {x2, y2}}, x, y) when x1 == x2 or y1 == y2 do
    min(x1,x2) <= x && x <= max(x1,x2) && min(y1,y2) <= y && y <= max(y1,y2)
  end

  defp intersects?({{x1, y1}, {x2, y2}}, x, y) do
