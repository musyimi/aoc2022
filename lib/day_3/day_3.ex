defmodule Aoc2022.Day3 do
  ...
  defp do_part_2(input) do
    ...
    Enum.reduce(input.numbers, {input.boards, nil}, fn number_being_called, {boards, winning_board} ->
     boards
     |> mark_number_as_called(number_being_called)
  ...
  defp score_board(winning_board, number_being_called) do
    ...
    Enum.reduce(winning_board, 0, fn {_coordinates, state}, acc ->
      if state.checked? do
        acc
      else
        acc + state.value
    ...
    Enum.map(board_strings, fn board_string ->
      ...
      |> Enum.reduce(%{}, fn {board_string, row_number}, board ->
       ...
       |>Enum.reduce(board, fn {value, col_number}, board ->
        Map.put(board, {row_number, col_number}, %{value: String.to_integer(value), checked?: false})
      end)
      end)
    end)
  end
